...
<div class="max-w-2xl">
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-400">Update an existing score.</p>
        </div>
    ...
    <form action="{{ route('scores.update', $score) }}" method="POST" class="space-y-6 rounded-3xl border border-gray-800 bg-gray-900 p-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="student_id" class="w-full rounded-2xl border border-gray-800 bg-gray-950 px-4 py-3 text-gray-200 focus:border-indigo-500 focus:outline-none" required>
                ...
                @foreach($students as $student)
                    <option value="{{ $student->id }}" {{ old('student_id', $score->student_id) == $student->id ? 'selected' : '' }}>{{ $student->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="subject_id" class="w-full rounded-2xl border border-gray-800 bg-gray-950 px-4 py-3 text-gray-200 focus:border-indigo-500 focus:outline-none" required>
                ...
                @foreach($subjects as $subject)
                    <option value="{{ $subject->id }}" {{ old('subject_id', $score->subject_id) == $subject->id ? 'selected' : '' }}>{{ $subject->subject }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="teacher_id" class="w-full rounded-2xl border border-gray-800 bg-gray-950 px-4 py-3 text-gray-200 focus:border-indigo-500 focus:outline-none" required>
                ...
                @foreach($teachers as $teacher)
                    <option value="{{ $teacher->id }}" {{ old('teacher_id', $score->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input type="number" name="score" value="{{ old('score', $score->score) }}" class="w-full rounded-2xl border border-gray-800 bg-gray-950 px-4 py-3 text-gray-200 focus:border-indigo-500 focus:outline-none" required min="0" max="100">
        </div>

        <button type="submit" class="rounded-2xl bg-indigo-600 px-6 py-3 text-white hover:bg-indigo-500">Update Score</button>
    </form>
</div>
@endsection
