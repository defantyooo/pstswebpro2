...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold">Subject Teachers</h1>
    <a href="{{ route('subject-teachers.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-500">Assign Teacher</a>
</div>
        ...
        <tbody class="divide-y divide-gray-800">
            @forelse($assignments as $assignment)
                <tr>
                    ...
                    <td class="px-6 py-4">{{ $assignment->subject->subject ?? '—' }}</td>
                    <td class="px-6 py-4">{{ $assignment->created_at->format('d M Y') }}</td>
                    <td class="px-6 py-4 space-x-2">
                        <form action="{{ route('subject-teachers.destroy', $assignment) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
